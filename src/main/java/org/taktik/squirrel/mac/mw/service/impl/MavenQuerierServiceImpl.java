...
public class MavenQuerierServiceImpl implements NexusQuerierService {
	...
	@Override
	public MavenPackage getLatestVersion(String groupId, String artifactId) throws VersionRangeResolutionException {
		...
		Version highestVersion = versions.stream().sorted(Comparator.reverseOrder()).filter(v -> !v.toString().endsWith("-SNAPSHOT")).findFirst().orElseThrow(() -> new IllegalArgumentException("Unknown artifact"));
	...
	public URI getUri(String groupId, String artifactId, String version) throws ArtifactResolutionException {
		...
		String url = getRepositoryUrl() +  groupId.replaceAll("\\.", "/") + "/" + artifactId.replaceAll("\\.", "/") + "/" + version + "/" + artifactId + "-" + version + ".zip";
